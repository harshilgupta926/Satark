...
# - resilient vision-model selection
...
def is_scam_claim(category, verdict, summary=""):
    """Detect an explicit scam claim without treating denials or uncertainty as one."""  # avoid false positives while detecting explicit scam claims.
...
# Keep image analysis on the currently documented Groq vision model.
# This avoids repeatedly calling legacy/inaccessible Llama 4 IDs and prevents
# the confusing model_not_found cascade shown in the previous implementation.
...
def analyze_with_groq(client, content, mode, role, image_data_urls=None, available_models=None):
    ...
    Vision requests intentionally use only Qwen 3.6 27B.  It is the currently
    documented Groq multimodal model for image input and JSON mode.  Text scans
    keep the existing SATARK text-model fallback chain.
...
"""
    ...
    def call(model, repair=False):
        ...
        if image_data_urls:
            ...
            # Qwen 3.6 supports non-thinking mode and JSON mode together.
            # Keeping reasoning out of message.content makes JSON parsing reliable.
    ...
    if image_data_urls:
        # Do not try legacy Llama 4 IDs. The user's current key rejects them,
        # and Qwen 3.6 27B is the current documented Groq vision + JSON model.
        candidates = [VISION_MODEL_PREFERENCES[0]]
    else:
    ...
    for model in candidates:
        try:
            ...
            # Vision has one deliberate model. Do not call inaccessible legacy
            # models after Qwen fails; report the real cause clearly instead.
            if image_data_urls:
                break
    ...
    if image_data_urls:
        raise RuntimeError(
            "SATARK could not complete the image/QR analysis with "
            "qwen/qwen3.6-27b. Please verify that this Groq API key/project "
            "has access to that vision model and try again.\n" + detail
        )
...
def render_result(result):
    ...
    with d: st.markdown(f'<div class="metric"><div class="metric-label">AI confidence</div><div class="metric-value">{confidence:.2f}%</div></div>',unsafe_allow_html=True)
...
def make_pdf_report(result, mode):
    ...
    meta=[[Paragraph("Scanner", body), Paragraph(pdf_escape(mode), body), Paragraph("Generated", body), Paragraph(datetime.now().strftime('%d %b %Y, %I:%M %p'), body)],
          ...
          [Paragraph("Pattern", body), Paragraph(pdf_escape(result.get('scam_pattern','Needs review')), body), Paragraph("AI confidence", body), Paragraph(f"{float(result.get('confidence',95.0)):.2f}%", body)]]

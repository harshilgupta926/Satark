...
.hero:before{
    content:"";position:absolute;width:280px;height:280px;left:50%;top:-240px;
    transform:translateX(-50%);border-radius:50%;background:rgba(255,255,255,.08);
...
.pill{
    position:relative;display:inline-flex;align-items:center;min-height:28px;padding:5px 12px;
    border:1px solid rgba(214,179,106,.36);border-radius:999px;color:#e8d5a7;
    background:rgba(214,179,106,.07);font-size:.62rem;font-weight:800;letter-spacing:.13em;
}
.hero h1{
    position:relative;margin:14px 0 8px;font-family:Sora,"Manrope",sans-serif;
    font-size:clamp(1.6rem,3.2vw,2.35rem);line-height:1.08;letter-spacing:-.03em;
    color:var(--text);
}
.hero h1 .hero-primary{display:inline-block;font-weight:500;color:#f3f3f6;font-size:.9em}
.hero h1 .hero-secondary{display:inline-block;font-weight:500;color:#e6e6eb;font-size:.9em}
.hero h1 .hero-brand{font-weight:800;color:#ffffff;background:linear-gradient(105deg,#ffffff 8%,#e2defe 55%,#a99cff 100%);-webkit-background-clip:text;background-clip:text;color:transparent}
.hero p{position:relative;max-width:640px;margin:0 auto;color:#bfc0c8;font-size:clamp(.82rem,1.3vw,.92rem);line-height:1.55}
.hero p strong{color:#f0f0f3;font-weight:700}
...
@media(max-width:768px){.block-container{padding:.7rem .65rem 3rem}.hero{padding:2.2rem .85rem;border-radius:18px}.hero h1{font-size:clamp(1.5rem,7vw,2rem);line-height:1.1}.hero p{font-size:.8rem}.scanner{min-height:82px;padding:9px}div[data-testid="stButton"]>button{width:100%}.result{padding:17px}}
</style>
""",
    unsafe_allow_html=True,
)
...
if st.session_state.page == "Analyze":
    ...
    if mode in {"Text","Email"}:
        ...
        video_file=st.file_uploader(
            ...
            help="SATARK extracts a handful of representative frames and, when possible, transcribes the audio. Max 200 MB.",
            key="video_input"
        )
        transcribe_audio=st.checkbox("Also transcribe and analyze the audio track (recommended for voice-call/scam-call videos)",value=True,key="video_transcribe_toggle")
        content="Analyze the sampled video frames (and transcript, if provided) together as one investigation."
    ...
    if mode in {"Image", "QR"}:
        ...
        current_input_fingerprint = single_file_fingerprint(video_file)
    else:
        current_input_fingerprint = ""

    if mode in {"Image", "QR", "Video"} and current_input_fingerprint != st.session_state.get("last_input_fingerprint", ""):
        st.session_state.last_input_fingerprint = current_input_fingerprint
        if current_input_fingerprint:
            st.session_state.result = None
    ...
    if analyze_clicked:
        ...
        try:
            # Each analysis receives a fresh request id and fresh model discovery.
            # This prevents a previous image/video scan from contaminating the next one.
            st.session_state.analysis_request_id = hashlib.sha256(
                f"{datetime.now().isoformat()}|{mode}".encode("utf-8")
            ).hexdigest()[:16]
            st.session_state.result = None
            ...
            with st.spinner("SATARK is reading the content, evaluating threat patterns and building your report…"):
                ...
                if mode in {"Text","Email"}:
                    ...
                    if video_file is None: raise ValueError("Please upload a video.")
                    if not _video_dependencies_available():
                        raise RuntimeError(
                            "Video analysis needs OpenCV installed in this environment "
                            "(pip install opencv-python-headless --break-system-packages), then restart the app."
                        )
                    frames, duration, warnings = extract_video_frames(video_file)
                    ...
                    duration_note = f"Approx. duration: {duration:.1f} seconds. " if duration else ""
                    transcript_note = (
